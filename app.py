...
def load_overall_analysis_details():
    ...
    with col2:
        ...
        st.metric(label='Maximum Funding size ',value = str(max_funding)+'cr')
    ...
    with col10:
        ...
        st.dataframe(df.groupby(['investors'])['amount'].sum().sort_values(ascending=False).reset_index().head(15))
...
def load_investor_details(investor):
    ...
    # 1. load recent 5 investments of the investor

    last5_df = df[df['investors'].str.contains(investor)].sort_values(by='date', ascending=False,ignore_index=True).head(10)[['date', 'startup', 'vertical', 'city', 'round', 'amount']]
    ...
    # stage -> pie
    with col3:
        stage_series = df[df['investors'].str.contains(investor)].groupby('round')['amount'].sum().sort_values(ascending=False).head(4)
    ...
    # Similar Investors
        ...
        l = df[df['investors'] == investor]['vertical'].value_counts().head(3).index.tolist()
        without_undis = df[df['investors'].str.contains('Undisclosed Investors', case=False)]
        new = df.drop(index = without_undis.index)
...
if option == 'Overall Analysis':
    ...
    btn3 = st.sidebar.button('Find Investor Details')
